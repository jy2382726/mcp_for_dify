...
支持三类接口：
- split_pdf_text: PDF 文本，先将 HTML 表格转换为 Markdown，再分块，父块末尾追加四个换行
- split_table_text: Markdown 表格文本，自动提取并复用表头，父块末尾追加四个换行
- split_text_with_preview_link: 纯文本，父块末尾追加图片链接地址与四个换行

架构思路：
- 预处理层：保护特殊标记（反引号内容与 URL），可选的 HTML->Markdown 表格转换、Markdown 表头提取
- 分段层：按句末标点与换行切分，保留原始分隔；边界规则可配置以保证与各实现一致
- 拼合层：
  - 父块：按父块大小逐段拼合（含原始分隔）
  - 子块：在父块内部按子块大小逐段拼合，并在子块之间插入三个换行
- 尾注层：在父块末尾统一追加四个换行，或追加图片链接地址再加四个换行

注意：不依赖第三方库，仅使用标准库；不修改原有文件，统一封装为可复用模块。
"""
...
def main(
    ...
    if m in ("pdf", "pdf_text"):
        splited_content = split_pdf_text(content, parent_block_size, sub_block_size)
    elif m in ("table", "md_table", "markdown"):
        splited_content = split_table_text(content, parent_block_size, sub_block_size)
        ...
        splited_content = split_text_with_preview_link(
...
def _protect_special_tokens(text: str) -> Tuple[str, List[str]]:
    tokens: List[str] = []

    def repl(m: re.Match) -> str:
        tokens.append(m.group(0))
        return f"{TOKEN_PREFIX}{len(tokens) - 1}{TOKEN_SUFFIX}"
...
def _restore_special_tokens(text: str, tokens: List[str]) -> str:
    for i, tok in enumerate(tokens):
        text = text.replace(f"{TOKEN_PREFIX}{i}{TOKEN_SUFFIX}", tok)
    return text
...
def _is_md_separator_row(line: str) -> bool:
    ...
    if not line:
        return False
    return bool(_MD_SEP_RE.match(line))


def _extract_md_header_prefix_and_body(text: str) -> Tuple[str, str, str]:
    ...
    header_start = None
    for i in range(n - 1):
        if _is_md_table_row(lines[i]) and _is_md_separator_row(lines[i + 1]):
            header_start = i
            break
...
def _split_into_units(text: str, boundary: re.Pattern) -> List[Tuple[str, str]]:
    if not text:
        return []
    text = text.replace("\r\n", "\n").replace("\r", "\n")
...
def _join_units_with_limit(
    units: List[Tuple[str, str]], limit: int
...
def _render_units(
    units: List[Tuple[str, str]], strip_last_sep_newlines: bool = False
) -> str:
    out: List[str] = []
    n = len(units)
...
def _apply_subblock_separators(
    parent_block_units: List[Tuple[str, str]], sub_limit: int
) -> str:
    subblocks = _join_units_with_limit(parent_block_units, sub_limit)
    rendered = [_render_units(sb, strip_last_sep_newlines=True) for sb in subblocks]
...
def split_pdf_text(content: str, parent_block_size: int, sub_block_size: int) -> str:
    ...
    converted = _convert_html_tables_to_markdown(content)
    protected_content, tokens = _protect_special_tokens(converted)

    effective_parent_size = max(1, parent_block_size)
    if sub_block_size < 1:
        sub_block_size = effective_parent_size

    units = _split_into_units(protected_content, BOUNDARY_PDF)
    parent_blocks = _join_units_with_limit(units, effective_parent_size)

    output_parts: List[str] = []
    for pblock in parent_blocks:
        body = _apply_subblock_separators(pblock, sub_block_size)
        body = body.rstrip("\n") + "\n\n\n\n"
        output_parts.append(body)

    final_text = "".join(output_parts)
    return _restore_special_tokens(final_text, tokens)


def split_table_text(content: str, parent_block_size: int, sub_block_size: int) -> str:
    ...
    protected_content, tokens = _protect_special_tokens(content)
    prefix_text, header_text, body_text = _extract_md_header_prefix_and_body(
        protected_content
    )
    ...
    header_units = (
        _split_into_units(header_text, BOUNDARY_GENERIC) if header_text else []
    )
    prefix_units = (
        _split_into_units(prefix_text, BOUNDARY_GENERIC) if prefix_text else []
    )
    body_units = _split_into_units(body_text, BOUNDARY_GENERIC)
    ...
    for pblock_units in parent_blocks_units:
        body = _apply_subblock_separators(pblock_units, sub_block_size)
        body = body.rstrip("\n") + "\n\n\n\n"
        output_parts.append(body)
...
def split_text_with_preview_link(
    content: str, preview_url: str, parent_block_size: int, sub_block_size: int
) -> str:
    ...
    units = _split_into_units(protected_content, BOUNDARY_GENERIC)
    parent_blocks = _join_units_with_limit(units, effective_parent_size)
